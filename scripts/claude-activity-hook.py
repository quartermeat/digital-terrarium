...
def paths(session_id):
    ...
    return identity, CONTROL / f"{identity}.json", AGENTS / f"claude-{identity}.json", CONTROL / f"{identity}.pid"
...
def handle(event):
    ...
    control = {"id": f"claude-{identity}", "phase": phase, "detail": detail, "target": target, "terminal": terminal, "updated": time.time()}
