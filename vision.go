...
// Face landmarks arrive from a separate renderer that owns the camera, so the
...
// Landmarks may sit slightly outside the frame when a face is half off camera;
// further than that is a bad report, not a reachable position.
...
func validVisionFrame(frame VisionFrame) bool {
	if frame.Version != 1 || !safeVisionName.MatchString(frame.Camera) ||
...
// A face describes a position held right now, so a feed that stops reporting
// must read as absent rather than leaving a phantom skull pinned to the last
// sighting. It expires the way in-progress agent phases do.
...
// The model is 3.6 MiB and the habitat runs all day, so fetch it once and serve
// the cached copy afterwards; a restart offline still starts vision.
